...
```
integrations/connector/v1/
├── definition/
│   ├── job.proto                  # Job, JobStatus, JobState
│   ├── external.proto             # ConnectorV1External (REST + gRPC)
│   └── internal.proto             # ConnectorV1Internal (HTTP + gRPC)
...
so both `ConnectorV1External` and `ConnectorV1Internal` gRPC services are registered on both
...
```
--integration.connector.v1                          Enable ConnectorV1 integration
--integration.connector.v1.internal                 Enable on internal listener (default: true)
--integration.connector.v1.external                 Enable on external listener (default: true)
--integration.connector.v1.connector-id             Connector UUID (required)
--integration.connector.v1.internal-address          Internal gRPC address for MetaV1/StorageV2 clients
```
...
```go
handler := connectorV1.New(metaClient, storageClient, connectorID)
```

- `metaClient` — MetaV1 gRPC client for job storage
- `storageClient` — StorageV2 gRPC client for file uploads
- `connectorID` — UUID from configuration (identifies the connector type)
...
Files uploaded to StorageV2 at path: `/connectors/<connector_id>/<job_id>/<filename>`
