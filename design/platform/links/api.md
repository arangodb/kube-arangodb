...
## LinkV1External — AI Tool / End User API
...
Submit a new job. The `query` field must match the link's JSON Schema.
...
## LinkV1Internal — Connector Process API

**Purpose**: Allows the link binary to claim jobs from the queue, report
progress, and upload results. This is the "producer" API.

**Audience**: Only the link process running inside the same pod. Accessible
...
| `external.proto` | `LinkV1External` service + request/response messages |
| `internal.proto` | `LinkV1Internal` service + request/response messages |
