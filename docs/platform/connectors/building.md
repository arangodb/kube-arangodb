...
A connector is a program that lets AI tools execute operations on external
systems (databases, APIs, services) through a standardized job-based interface.

## What Is a Connector?

A connector runs as a container inside a Kubernetes pod. The platform injects
an integration sidecar into the pod that provides:
- A **job queue** (MetaStore) — where submitted jobs wait to be processed
- A **file store** (StorageV2) — where the connector uploads results
- An **internal HTTP API** at `http://127.0.0.1:9192` — how the connector
  communicates with the platform

Your connector binary is a simple loop: pick up a job, execute it, upload
...
- A **query** — JSON input matching the connector's schema
...
2. An `ArangoPlatformConnector` that registers it with the platform
...
validates submitted jobs against this schema — your connector does not need
to validate the schema itself, but may optionally do so for defense in depth.

## Step 2: Implement the Connector Loop

Your connector binary polls the internal API for jobs and processes them:
...
If your connector does not check for cancellation, a cancelled job will
...
Your Helm chart is the connector's own packaging — it deploys the connector
...
├── templates/
│   ├── connector.yaml      # ArangoPlatformConnector CRD
...
See the [sample AQL connector](https://github.com/arangodb/kube-arangodb-test/tree/master/tests/connectors/aql)
