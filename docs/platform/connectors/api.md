...
`ArangoRoute` configured for the connector.

Each connector instance handles its own jobs — when you submit a job, it goes
to the connector whose endpoint you called. There is no routing between
connectors.
...
Submit work to the connector. The `query` field is a JSON object whose
structure must match the connector's `schema` (published in `/_inventory`).
...
- `query` — JSON string matching the connector's schema. Sent as a string
  (the proto field is `bytes`, which serializes as base64 over JSON).
- `timeout` — optional, maximum time the connector has to complete the job.
...
where the connector uploaded its output files.
...
## Internal API — for the Connector Process

The Internal API is used by the connector binary running inside the pod.
...
payload that the connector needs to execute.
