...
TEST_BUILD ?= 0
GOBUILDARGS ?=
GOBASEVERSION := 1.26.8
GOVERSION ?= $(GOBASEVERSION)-alpine3.18
DISTRIBUTION ?= alpine:3.15
GOCOMPAT := $(shell sed -En 's/^go (.*)$$/\1/p' go.mod)

# Pinned by tag, not by GitHub release: golang/vuln stopped publishing releases at
# v1.1.4 while continuing to tag, so anything resolving "latest" from the releases
# API reports v1.1.4. Check the tag list or the module proxy when bumping.
...
YAML_EXCLUDE_DIRS := vendor .gobuild deps tools pkg/generated/clientset pkg/generated/informers pkg/generated/listers \
                     ...
                     chart/kube-arangodb-crd/templates chart/platform-storage/templates chart/arango-deployment/templates pkg/platform/templates
YAML_EXCLUDE_FILES :=
...
# Own target so a job needing only the scanner skips the whole toolchain, and so
# the version pin has one home.
...
CRDS:=backups-backup backups-backuppolicy \
      ...
      networking-route \
      platform-storage platform-chart platform-service platform-link platform-workflow \
