...
func Releases(log *logging.Logger, user, repo, token string) ([]Release, error) {
	...
	err := GithubGet(log, fmt.Sprintf(RELEASE_LIST_URI, user, repo, token), &releases)
	if err != nil {
		return nil, err
	}
...
func latestReleaseApi(log *logging.Logger, user, repo, token string) (*Release, error) {
	if token != "" {
		token = "?access_token=" + token
	}
	var release Release
	return &release, GithubGet(log, fmt.Sprintf(RELEASE_LATEST_URI, user, repo, token), &release)
}
...
func IdOfTag(log *logging.Logger, user, repo, tag, token string) (int, error) {
	release, err := ReleaseOfTag(log, user, repo, tag, token)
	if err != nil {
		return 0, err
	}
