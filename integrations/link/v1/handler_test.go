...
func Test_Heartbeat_Registers(t *testing.T) {
	...
	go func() {
		defer close(done)
		startHeartbeat(ctx, meta, linkID, handlerID)
	}()
...
func Test_Heartbeat_MultipleHandlers(t *testing.T) {
	...
	go func() {
		defer close(done1)
		startHeartbeat(ctx, meta, linkID, handler1)
	}()

	done2 := make(chan struct{})
	go func() {
		defer close(done2)
		startHeartbeat(ctx, meta, linkID, handler2)
	}()

	time.Sleep(100 * time.Millisecond)

	// Both handlers should be registered
	_, err := meta.Get(context.Background(), &pbMetaV1.ObjectRequest{Key: handlerKey(linkID, handler1)})
	require.NoError(t, err)

	_, err = meta.Get(context.Background(), &pbMetaV1.ObjectRequest{Key: handlerKey(linkID, handler2)})
	require.NoError(t, err)
...
func Test_Heartbeat_StopsOnCancel(t *testing.T) {
	...
	go func() {
		defer close(done)
		startHeartbeat(ctx, meta, testLinkID, handlerID)
	}()
...
func Test_Heartbeat_NotRegisteredBeforeStart(t *testing.T) {
	...
	key := handlerKey(testLinkID, handlerID)
