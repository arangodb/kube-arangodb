...
func testEncryptionBatch(t *testing.T, size int) {
	t.Run(fmt.Sprintf("%d", size), func(t *testing.T) {
		...
		t.Run("Get encrypted - invalid password", func(t *testing.T) {
			...
			require.EqualError(t, err, "rpc error: code = FailedPrecondition desc = Object /data decryption failed: cipher: message authentication failed")
			require.Nil(t, resp)
		})
		...
		t.Run("Get encrypted - missing password", func(t *testing.T) {
			resp, err := client.Get(ctx, &pbMetaV1.ObjectRequest{Key: "/data"})
			require.EqualValues(t, codes.FailedPrecondition, errors.GRPCCode(err))
			require.EqualError(t, err, "rpc error: code = FailedPrecondition desc = Object /data encrypted, but secret is missing")
			require.Nil(t, resp)
		})
	})
}
