...
func getClient(t *testing.T) pbImplStorageV2Shared.IO {
	...
	cfg.Client = tests.GetGCSClientConfig(t)
	cfg.BucketName = tests.GetGCSBucket(t)
	cfg.BucketPrefix = fmt.Sprintf("test/%s/", uuid.NewUUID())

	z, err := cfg.New(shutdown.Context())
...
func Test(t *testing.T) {
	...
	err := w.Init(ctx, &pbImplStorageV2Shared.InitOptions{
		Create: util.NewType(true),
	})
	require.NoError(t, err)
