...
// defaultWorkflowMaxHistory is the Helm release history kept for workflow-managed releases when the
// sidecar has no configured/explicit MaxHistory. It matches the ArangoPlatformWorkflow CRD default and
// must be >= 1 (the CRD rejects 0).
const defaultWorkflowMaxHistory = 10

// workflowRequiredVerbs are the verbs the sidecar performs on ArangoPlatformWorkflow resources when the
// scheduler-v2-workflow feature is enabled (Install/Upgrade Get+Create+Update, Uninstall Delete).
var workflowRequiredVerbs = []string{"get", "create", "update", "delete"}
...
// checkWorkflowPermissions verifies via SelfSubjectAccessReview that the sidecar's ServiceAccount may
// perform every verb it needs on ArangoPlatformWorkflow resources in its namespace. It returns an error
// listing the missing verbs so a misconfigured RBAC is reported up front instead of failing on the first
// Install/Upgrade/Uninstall with a raw "forbidden".
func (i *implementation) checkWorkflowPermissions(ctx context.Context) error {
	missing := make([]string, 0, len(workflowRequiredVerbs))

	for _, verb := range workflowRequiredVerbs {
		review, err := i.kclient.Kubernetes().AuthorizationV1().SelfSubjectAccessReviews().Create(ctx, &authorizationApi.SelfSubjectAccessReview{
			Spec: authorizationApi.SelfSubjectAccessReviewSpec{
				ResourceAttributes: &authorizationApi.ResourceAttributes{
					Namespace: i.cfg.Namespace,
					Verb:      verb,
					Group:     platform.ArangoPlatformGroupName,
					Resource:  platform.ArangoPlatformWorkflowResourcePlural,
				},
			},
		}, meta.CreateOptions{})
		if err != nil {
			return errors.Wrapf(err, "unable to run SelfSubjectAccessReview for verb %s on %s", verb, platform.ArangoPlatformWorkflowResourcePlural)
		}
	...
	if len(missing) > 0 {
		return errors.Errorf("ServiceAccount is not allowed to [%s] %s in namespace %s", goStrings.Join(missing, ", "), platform.ArangoPlatformWorkflowResourcePlural, i.cfg.Namespace)
	}
...
func (i *implementation) upgradeV2Workflow(ctx context.Context, in *pbSchedulerV2.SchedulerV2UpgradeV2Request) (*pbSchedulerV2.SchedulerV2UpgradeV2Response, error) {
	...
	// The CRD requires MaxHistory >= 1, but the sidecar's configured MaxHistory may be unset (0).
	// Default to defaultWorkflowMaxHistory so the workflow spec is always valid.
	if maxHistory < 1 {
		maxHistory = defaultWorkflowMaxHistory
	}
