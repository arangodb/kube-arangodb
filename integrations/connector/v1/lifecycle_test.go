...
func Test_Lifecycle_Complete(t *testing.T) {
	...
	requireJobState(t, impl, id, pbConnectorV1.JobState_JOB_STATE_PENDING)
	...
	// 3. Running
	updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_RUNNING, "Executing AQL query")
	requireJobState(t, impl, id, pbConnectorV1.JobState_JOB_STATE_RUNNING)
	...
	// 5. Complete
	job = updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_COMPLETED, "Query returned 2 documents")
...
func Test_Lifecycle_Failed(t *testing.T) {
	...
	job := updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_FAILED, "AQL parse error at position 1:8")
...
func Test_Lifecycle_CancelWhileRunning(t *testing.T) {
	...
	pickUp(t, impl)
	updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_RUNNING, "Executing long query")

	resp, err := impl.CancelJob(ctx, &pbConnectorV1.CancelJobRequest{Id: id})
	require.NoError(t, err)

	requireStatusHistory(t, resp.Job,
		pbConnectorV1.JobState_JOB_STATE_CANCELLED,
		pbConnectorV1.JobState_JOB_STATE_RUNNING,
		pbConnectorV1.JobState_JOB_STATE_SCHEDULED,
		pbConnectorV1.JobState_JOB_STATE_PENDING,
	)
...
func Test_Lifecycle_MultipleJobs(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if i < 2 {
			...
			updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_RUNNING, "Running")
			updateStatus(t, impl, id, pbConnectorV1.JobState_JOB_STATE_FAILED, "Error")
		}
	}
	...
	// Count by state
	completed := pbConnectorV1.JobState_JOB_STATE_COMPLETED
	respCompleted, err := impl.ListJobs(ctx, &pbConnectorV1.ListJobsRequest{State: &completed})
	require.NoError(t, err)
	require.Len(t, respCompleted.Jobs, 2)

	failed := pbConnectorV1.JobState_JOB_STATE_FAILED
	respFailed, err := impl.ListJobs(ctx, &pbConnectorV1.ListJobsRequest{State: &failed})
	require.NoError(t, err)
