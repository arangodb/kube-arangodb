...
const testConnectorID = "00000000-0000-0000-0000-000000000001"

func newTestImpl(t *testing.T) *implementation {
	return New(testIntegration.NewMetaV1Client(), testIntegration.NewStorageV2Client(t), testConnectorID).(*implementation)
}
...
func requireJobState(t *testing.T, impl *implementation, id string, state pbConnectorV1.JobState) *pbConnectorV1.Job {
	t.Helper()

	resp, err := impl.GetJob(context.Background(), &pbConnectorV1.GetJobRequest{Id: id})
	require.NoError(t, err)
	require.Equal(t, state, currentState(resp.Job))
	return resp.Job
}

func requireStatusHistory(t *testing.T, job *pbConnectorV1.Job, states ...pbConnectorV1.JobState) {
	t.Helper()
...
func updateStatus(t *testing.T, impl *implementation, id string, state pbConnectorV1.JobState, desc string) *pbConnectorV1.Job {
	t.Helper()

	resp, err := impl.UpdateJobStatus(context.Background(), &pbConnectorV1.UpdateJobStatusRequest{
		Id: id,
