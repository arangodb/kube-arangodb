...
func (s *stateInspector) fetchGatewayMemberState(ctx context.Context, m api.DeploymentStatusMemberElement) State {
	...
	// Use raw request to fetch version from the gateway. The gateway proxies
	// /_api/version through Envoy, which may return non-JSON error responses
	// (e.g. 503) when the backend is not yet ready. The go-driver v2
	// Client.Version() cannot decode non-JSON responses, so we use the
	// request module which handles this gracefully.
	v, err := arangod.GetRequest[adbDriverV2.VersionInfo](ctx, c.Connection(), "_api", "version").
		Do(ctx).
