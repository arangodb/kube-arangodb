...
func (r *Reconciler) createRotateServerStoragePVCPendingResizeConditionPlan(ctx context.Context, apiObject k8sutil.APIObject,
	...
	for _, i := range status.Members.AsList() {
		...
		if pvcResizePending != pvcResizePendingCond {
			if pvcResizePending {
				plan = append(plan, sharedReconcile.UpdateMemberConditionActionV2("PVC Resize pending", api.ConditionTypePVCResizePending, i.Group, i.Member.ID, true, "PVC Resize pending", "", ""))
			} else {
				plan = append(plan, sharedReconcile.RemoveMemberConditionActionV2("PVC Resize is done", api.ConditionTypePVCResizePending, i.Group, i.Member.ID))
			}
		}
	}
