...
func (r *Reconciler) createCleanOutPlan(ctx context.Context, _ k8sutil.APIObject, spec api.DeploymentSpec,
	...
	for _, m := range status.Members.AsListInGroup(api.ServerGroupDBServers) {
		...
		if cleanedStatus != cleanedAgency {
			if cleanedAgency {
				plan = append(plan, sharedReconcile.UpdateMemberConditionActionV2("DBServer cleaned", api.ConditionTypeCleanedOut, m.Group, m.Member.ID, true, "DBServer cleaned", "DBServer cleaned", ""))
			} else {
				plan = append(plan, sharedReconcile.RemoveMemberConditionActionV2("DBServer is not cleaned", api.ConditionTypeCleanedOut, m.Group, m.Member.ID))
			}
		}
	}
