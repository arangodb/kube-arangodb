	...
	for _, m := range status.Members.AsList() {
		...
		if !recreate {
			if _, ok := m.Member.Conditions.Get(api.MemberReplacementRequired); ok {
				// Unset condition
				p = append(p, sharedReconcile.RemoveMemberConditionActionV2("Member replacement not required", api.MemberReplacementRequired, m.Group, m.Member.ID))
			}
		} else {
			if c, ok := m.Member.Conditions.Get(api.MemberReplacementRequired); !ok || !c.IsTrue() || c.Message != message {
				// Update condition
				p = append(p, sharedReconcile.UpdateMemberConditionActionV2("Member replacement required", api.MemberReplacementRequired, m.Group, m.Member.ID, true, "Member replacement required", message, ""))
			}
		}
	}
