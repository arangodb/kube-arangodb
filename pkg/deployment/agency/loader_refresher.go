...
func RefreshLoader[T interface{}](loader leader.StateLoader[T], delay time.Duration) leader.StateLoader[T] {
	if delay <= 0 {
		return loader
	}
...
func (i *refresherLoader[T]) Refresh(ctx context.Context, discovery leader.Discovery) error {
	i.lock.Lock()
	defer i.lock.Unlock()
