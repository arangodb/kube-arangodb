...
func NewAgencyApplier[T interface{}](cfg ApplierConfig) leader.StateLoader[T] {
...
func (a *agencyApplier[T]) Refresh(ctx context.Context, discovery leader.Discovery) (err error) {
	a.lock.Lock()
	defer a.lock.Unlock()
	defer func() {
		if err != nil {
	...
	defer func() {
		// Catch panic
		if p := recover(); p != nil {
			err = errors.WithStack(errors.Errorf("Panic recovery: %+v", p))
		}
	}()
	...
	if resp.Result.FirstIndex == nil {
		...
		if resp.Result.ReadDB == nil {
			// There is no data
			return errors.Errorf("Data is missing")
		}
