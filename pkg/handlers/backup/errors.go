...
func newTemporaryErrorf(format string, a ...interface{}) error {
	return newTemporaryError(errors.Errorf(format, a...))
}
...
func newFatalErrorf(format string, a ...interface{}) error {
	return newFatalError(errors.Errorf(format, a...))
}
...
func isTemporaryError(err error) bool {
	...
	// Check error string
	if goStrings.Contains(err.Error(), "context deadline exceeded") {
		return true
	}

	if goStrings.Contains(err.Error(), "connection refused") {
		return true
	}
