...
func stateUploadingHandler(h *handler, backup *backupApi.ArangoBackup) (*backupApi.ArangoBackupStatus, error) {
	...
	details, err := client.Progress(backup.Status.Progress.JobID, adbDriverV2.TransferTypeUpload)
	if err != nil {
	...
	if backup.Spec.Upload == nil {
		...
		if err = client.Abort(backup.Status.Progress.JobID, adbDriverV2.TransferTypeUpload); err == nil {
			return wrapUpdateStatus(backup,
				updateStatusState(backupApi.ArangoBackupStateReady, ""),
				cleanStatusJob(),
