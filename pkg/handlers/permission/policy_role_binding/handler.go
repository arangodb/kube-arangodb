...
func (h *handler) Handle(ctx context.Context, item operation.Item) error {
	object, err := util.WithKubernetesContextTimeoutP2A2(ctx, h.client.PermissionV1alpha1().ArangoPermissionPolicyRoleBindings(item.Namespace).Get, item.Name, meta.GetOptions{})
	if err != nil {
	...
	if object.GetDeletionTimestamp() != nil {
		if changed, err := patcher.EnsureFinalizersGone(ctx, h.client.PermissionV1alpha1().ArangoPermissionPolicyRoleBindings(item.Namespace), object,
			permissionApi.FinalizerArangoPermissionPolicyRoleBinding,
		); err != nil {
			return err
		} else if changed {
	...
	if changed, err := patcher.EnsureFinalizersPresent(ctx, h.client.PermissionV1alpha1().ArangoPermissionPolicyRoleBindings(item.Namespace), object,
		permissionApi.FinalizerArangoPermissionPolicyRoleBinding,
	); err != nil {
		return err
	} else if changed {
	...
	if _, err := operator.WithArangoPermissionPolicyRoleBindingUpdateStatusInterfaceRetry(context.Background(), h.client.PermissionV1alpha1().ArangoPermissionPolicyRoleBindings(object.GetNamespace()), object, *status, meta.UpdateOptions{}); err != nil {
		return err
	}
...
func (h *handler) handle(ctx context.Context, item operation.Item, extension *permissionApi.ArangoPermissionPolicyRoleBinding, status *permissionApi.ArangoPermissionPolicyRoleBindingStatus) (bool, error) {
...
func (h *handler) HandleSpecValidity(ctx context.Context, item operation.Item, extension *permissionApi.ArangoPermissionPolicyRoleBinding, status *permissionApi.ArangoPermissionPolicyRoleBindingStatus) (bool, error) {
...
func (h *handler) HandleDeployment(ctx context.Context, item operation.Item, extension *permissionApi.ArangoPermissionPolicyRoleBinding, status *permissionApi.ArangoPermissionPolicyRoleBindingStatus) (bool, error) {
	...
	return operator.HandleP4(ctx, item, extension, status, depl, h.HandleReferences)
}
