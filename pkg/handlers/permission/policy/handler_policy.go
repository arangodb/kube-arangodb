...
func (h *handler) HandleArangoDBPolicy(ctx context.Context, item operation.Item, extension *permissionApi.ArangoPermissionPolicy, st *permissionApi.ArangoPermissionPolicyStatus, depl *api.ArangoDeployment, conn sidecarSvcAuthzDefinition.AuthorizationAPIClient) (bool, error) {
	...
	name := fmt.Sprintf("managed:operator:%s", extension.GetName())
...
func (h *handler) finalizerPolicyRemoval(ctx context.Context, extension *permissionApi.ArangoPermissionPolicy) error {
