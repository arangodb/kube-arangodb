...
func Test_ChartReconcile_EmptyChart(t *testing.T) {
	...
	t.Run("Valid chart 1.0.0", func(t *testing.T) {
		...
		require.EqualValues(t, util.SHA256FromStringArray(util.SHA256(suite.GetChart(t, "example", "1.0.0"))), extension.Status.Info.Checksum)
	...
	t.Run("Valid chart 1.0.1", func(t *testing.T) {
		...
		require.EqualValues(t, util.SHA256FromStringArray(util.SHA256(suite.GetChart(t, "example", "1.0.1"))), extension.Status.Info.Checksum)
	...
	t.Run("Valid chart 1.1.0", func(t *testing.T) {
		...
		require.Nil(t, extension.Status.Info.Details.Platform)
		require.EqualValues(t, util.SHA256FromStringArray(util.SHA256(suite.GetChart(t, "example", "1.1.0"))), extension.Status.Info.Checksum)
		require.True(t, extension.Status.Conditions.IsTrue(platformApi.ReadyCondition))
	})

	t.Run("Valid chart 1.1.0 with overrides", func(t *testing.T) {
		// Arrange
		type object struct {
		...
		require.EqualValues(t, "1.1.0", extension.Status.Info.Details.GetVersion())
		require.Nil(t, extension.Status.Info.Details.Platform)
		require.EqualValues(t, util.SHA256FromStringArray(util.SHA256(suite.GetChart(t, "example", "1.1.0")), data.SHA256()), extension.Status.Info.Checksum)
		require.True(t, extension.Status.Conditions.IsTrue(platformApi.ReadyCondition))
		require.NotNil(t, extension.Status.Info.Overrides)

		nObj, err := sharedApi.FromAny[object](extension.Status.Info.Overrides)
		require.NoError(t, err)
		require.Equal(t, obj, nObj)
	})
}
