...
func Test_Event_Handler_Normal(t *testing.T) {
	// Arrange
	...
	// Act
	instance.Normal(p, reason, message)
	...
	assert.Equal(t, core.EventTypeNormal, event.Type)
...
func Test_Event_Handler_Warning(t *testing.T) {
	// Arrange
	c := fake.NewSimpleClientset()
	...
	// Act
	instance.Warning(p, reason, message)
	...
	event := events.Items[0]
	assert.Equal(t, core.EventTypeWarning, event.Type)
	assert.Equal(t, reason, event.Reason)
	assert.Equal(t, message, event.Message)
	assert.Equal(t, "mock", event.Source.Component)
