...
func serviceEnable() (*cobra.Command, error) {
	var cmd cobra.Command

	cmd.Use = "enable [flags] deployment name chart"
	...
	cmd.RunE = getRunner().With(serviceEnableRun).Run

	return &cmd, nil
}

func serviceEnableRun(cmd *cobra.Command, args []string) error {
	...
	deployment, name, chart := args[0], args[1], args[2]

	logger := logger.Str("deployment", deployment).Str("name", name).Str("chart", chart)
	...
	chartObject, err := waitForChart(cmd.Context(), client, ns, name).Run(cmd.Context(), time.Minute, time.Second)
	if err != nil {
		return err
	}
