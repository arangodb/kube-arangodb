...
func ExtractDeploymentID(ctx context.Context, conn adbDriverV2Connection.Connection) (string, error) {
	...
	health, err := arangod.GetRequestWithTimeout[adbDriverV2.ClusterHealth](ctx, globals.GetGlobals().Timeouts().ArangoD().Get(), conn, "_admin", "cluster", "health").AcceptCode(goHttp.StatusOK).Response()
	if err == nil {
