...
func licenseActivate() (*cobra.Command, error) {
	...
	if err := cli.RegisterFlags(&cmd, flagLicenseManagerEndpoint, flagLicenseManagerClientID, flagLicenseManagerClientSecret, flagArangoDBEndpoint, flagActivateInterval); err != nil {
		return nil, err
	}
...
func licenseActivateRun(cmd *cobra.Command, args []string) error {
	...
	conn, err := arangoDBConnection(cmd)
	if err != nil {
		return err
	}
...
func licenseActivateExecute(ctx context.Context, logger logging.Logger, c client.Client, mc manager.Client) error {
	...
	lic, err := mc.License(ctx, manager.LicenseRequest{
		...
		TTL:          util.NewType(util.Duration(time.Hour)),
	})
	if err != nil {
		return err
	}
