...
var requestLogger = logging.Global().RegisterAndGetLogger("arangod-request", logging.Info)
...
func (r *request[OUT]) Do(ctx context.Context) Response[OUT] {
	...
	if err != nil {
		...
		// Guard against typed nil interface values (e.g. (*httpResponse)(nil))
		// which pass a plain != nil check but panic on method calls.
...
func (r response[OUT]) HTTPResponse() (*goHttp.Response, error) {
	if r.resp == nil {
		return nil, errors.Errorf("Missing HTTP Response")
	}
