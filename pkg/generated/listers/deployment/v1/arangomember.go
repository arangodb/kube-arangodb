...
type ArangoMemberLister interface {
	...
	List(selector labels.Selector) (ret []*deploymentv1.ArangoMember, err error)
...
type arangoMemberLister struct {
	listers.ResourceIndexer[*deploymentv1.ArangoMember]
}
...
func NewArangoMemberLister(indexer cache.Indexer) ArangoMemberLister {
	return &arangoMemberLister{listers.New[*deploymentv1.ArangoMember](indexer, deploymentv1.Resource("arangomember"))}
}

// ArangoMembers returns an object that can list and get ArangoMembers.
func (s *arangoMemberLister) ArangoMembers(namespace string) ArangoMemberNamespaceLister {
	return arangoMemberNamespaceLister{listers.NewNamespaced[*deploymentv1.ArangoMember](s.ResourceIndexer, namespace)}
}
...
type ArangoMemberNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*deploymentv1.ArangoMember, error)
