...
- (Feature) Inject the Pod UID as the MY_POD_UID lifecycle env into ArangoDB and sidecar containers (allowlisted in the rotation compare so adding it does not trigger a member rotation) and tag the collector startup event with a podUID dimension
